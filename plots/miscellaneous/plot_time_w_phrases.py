...
def plot_file_numbers(directories, extension, output_basename, sample, multiplier):
    ...
    for software, files in software_groups.items():
        ...
        # Collect the times for only the file numbers that are available for this software
        ...
        if sorted_phrases:  # Only plot if there are valid numbers
            plt.plot(sorted_phrases, times, marker='o', linestyle='-', label=software)

    # Plot update interval tree time
    plt.plot(sorted_phrases, update_tree_time, marker='o', linestyle='-', label="update interval tree")
...
# Example usage
plot_file_numbers(["/blue/boucher/rvarki/rlz-repair_analysis/benchmarks/rlz-repair/SARS/hpg-milan"], 
                    ".compress.benchmark.txt",
                    "sars_time_rlz_phrases",
                    "SARS-CoV-2",
                    False)
